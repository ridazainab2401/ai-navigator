...
export type GestureType = 'thumbs_up' | 'peace' | 'open_palm' | 'pointing' | 'closed_fist' | 'none';
...
interface GestureState {
  ...
  selectProgress: number; // 0..1 while dwelling
}
...
export function useGestureDetection(options: UseGestureDetectionOptions = {}) {
  const { onGestureDetected, onSelect, cooldownMs = 1000 } = options;
  ...
  // Cursor + dwell-to-select
  const SELECT_DWELL_MS = 650;
  const MOVE_TOLERANCE_PX = 80;
  const POINTER_SMOOTHING = 0.25; // 0..1 (higher = less smoothing)
  const POINTER_HIDE_GRACE_MS = 600;
  const selectStartTimeRef = useRef<number | null>(null);
  ...
  const motionEwmaRef = useRef<number>(0);
  const selectTriggeredSinceReleaseRef = useRef<boolean>(false);
  const selectReleaseFramesRef = useRef<number>(0);
  ...
  const detectGestureFromLandmarks = useCallback((hands: HandLandmarks[], handedness: HandednessLabel): GestureType => {
    ...
    const isFingerExtended = (tip: Landmark, pip: Landmark, wristPoint: Landmark) => {
    ...
    const thumbExtended = handedness === 'Left' ? thumb_tip.x > thumb_ip.x : thumb_tip.x < thumb_ip.x;
  ...
  const onResults = useCallback((results: Results) => {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      const handednessLabel: HandednessLabel =
        results.multiHandedness?.[0]?.label === 'Left' || results.multiHandedness?.[0]?.label === 'Right'
          ? (results.multiHandedness?.[0]?.label as HandednessLabel)
          : 'Unknown';

      const rawGesture = detectGestureFromLandmarks(results.multiHandLandmarks as HandLandmarks[], handednessLabel);
      ...
      const wrist = hand0?.[0];
      const indexLikelyExtended = !!(indexTip && indexPip && wrist && indexTip.y < indexPip.y && indexTip.y < wrist.y);
      ...
      if (indexLikelyExtended && indexTip) {
        ...
        lastMotionDeltaRef.current = prevMotion ? Math.hypot(smooth.x - prevMotion.x, smooth.y - prevMotion.y) : 0;
        motionEwmaRef.current = motionEwmaRef.current * 0.8 + lastMotionDeltaRef.current * 0.2;
      ...
      if (sameGestureFramesRef.current >= STABLE_FRAMES && rawGesture !== stableGestureRef.current) {
        stableGestureRef.current = rawGesture;
        setState((prev) => ({
          ...prev,
          gesture: rawGesture,
          confidence: Math.min(1, sameGestureFramesRef.current / STABLE_FRAMES),
        }));
      }
      ...
      if (stableGestureRef.current === 'none') {
        ...
        if (noneFramesRef.current >= RELEASE_FRAMES) {
          ...
          selectTriggeredSinceReleaseRef.current = false;
        }
      } else {
        noneFramesRef.current = 0;
      }

      // Dwell-to-select (pointing): hold cursor steady to click once
      const now = Date.now();
      const stable = stableGestureRef.current;
      // Re-arm click once the user stops pointing for a bit
      if (rawGesture !== 'pointing') {
        selectReleaseFramesRef.current += 1;
        if (selectReleaseFramesRef.current >= 4) {
          selectTriggeredSinceReleaseRef.current = false;
        }
      } else {
        selectReleaseFramesRef.current = 0;
      }

      // Use raw 'pointing' (not the stabilized gesture) for click, so small classifier noise doesn't block clicking.
      if (rawGesture === 'pointing' && onSelect && lastPointerRef.current) {
        if (selectTriggeredSinceReleaseRef.current) {
          if (state.selectProgress !== 0) {
            setState((prev) => ({ ...prev, selectProgress: 0 }));
          }
        } else {
          const last = lastPointerRef.current;

          if (motionEwmaRef.current <= MOVE_TOLERANCE_PX) {
            if (selectStartTimeRef.current == null) {
              selectStartTimeRef.current = now;
            }
            const progress = Math.max(0, Math.min(1, (now - selectStartTimeRef.current) / SELECT_DWELL_MS));
            if (progress !== state.selectProgress) {
              setState((prevState) => ({ ...prevState, selectProgress: progress }));
            }
            if (progress >= 1) {
              selectTriggeredSinceReleaseRef.current = true;
              selectStartTimeRef.current = null;
              setState((prevState) => ({ ...prevState, selectProgress: 0 }));
              onSelect(last);
            }
          } else {
            selectStartTimeRef.current = null;
      ...
      // Trigger once per hold + cooldown (non-pointing gestures)
      if (stable !== 'none' && stable !== 'pointing' && !triggeredSinceReleaseRef.current && now - lastGestureTime.current > cooldownMs) {
      ...
      if (sameGestureFramesRef.current >= STABLE_FRAMES && stableGestureRef.current !== 'none') {
      ...
      if (now - lastHandSeenAtRef.current > POINTER_HIDE_GRACE_MS) {
        if (state.pointer || state.selectProgress !== 0) {
          lastPointerRef.current = null;
          smoothedPointerRef.current = null;
          prevPointerMotionRef.current = null;
          lastMotionDeltaRef.current = 0;
          motionEwmaRef.current = 0;
          selectStartTimeRef.current = null;
          setState((prev) => ({ ...prev, pointer: null, selectProgress: 0 }));
        }
      }
    }
  }, [cooldownMs, detectGestureFromLandmarks, onGestureDetected, onSelect, state.pointer, state.selectProgress]);
