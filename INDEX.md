# 📖 AI CoE Gesture Website - Documentation Index

Welcome! This is your guide to all project documentation.

## 🚀 Getting Started (Pick One)

| If you want to... | Read this file |
|------------------|----------------|
| **Start in 30 seconds** | [QUICKSTART.md](QUICKSTART.md) |
| **Detailed setup (3 steps)** | [SETUP_GUIDE.md](SETUP_GUIDE.md) |
| **Complete documentation** | [README.md](README.md) |

## 🎮 Using the Website

| If you want to... | Read this file |
|------------------|----------------|
| **Learn gesture controls** | [DEMO_GUIDE.md](DEMO_GUIDE.md) |
| **Prepare for demo/evaluation** | [DEMO_GUIDE.md](DEMO_GUIDE.md) |

## 📋 Project Information

| If you want to... | Read this file |
|------------------|----------------|
| **Check requirements met** | [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md) |
| **Understand project structure** | [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) |
| **Full technical docs** | [README.md](README.md) |

## 📄 All Documentation Files

### 1. [QUICKSTART.md](QUICKSTART.md)
**⏱️ Read time: 1 minute**
- Fastest way to get started
- Two setup options
- Quick troubleshooting

### 2. [SETUP_GUIDE.md](SETUP_GUIDE.md)
**⏱️ Read time: 3 minutes**
- Detailed 3-step setup
- Installation instructions
- Common issues and solutions
- Testing checklist

### 3. [README.md](README.md)
**⏱️ Read time: 10 minutes**
- Complete project documentation
- Features overview
- Technology stack
- Build instructions
- Troubleshooting guide

### 4. [DEMO_GUIDE.md](DEMO_GUIDE.md)
**⏱️ Read time: 5 minutes**
- Gesture controls reference
- How to use gestures
- Tips for best results
- Demo procedure
- Recording/presentation tips

### 5. [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)
**⏱️ Read time: 8 minutes**
- Requirements checklist ✅
- Architecture overview
- Technology choices
- Features breakdown
- Evaluation guide

### 6. [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)
**⏱️ Read time: 7 minutes**
- Complete file tree
- Key files explained
- Component hierarchy
- Dependency overview

## 🎯 Quick Navigation by Role

### 👨‍💻 Developer
1. Start: [QUICKSTART.md](QUICKSTART.md)
2. Setup: [SETUP_GUIDE.md](SETUP_GUIDE.md)
3. Code: [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)
4. Full docs: [README.md](README.md)

### 👨‍🏫 Evaluator
1. Requirements: [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)
2. Demo prep: [DEMO_GUIDE.md](DEMO_GUIDE.md)
3. Setup: [SETUP_GUIDE.md](SETUP_GUIDE.md)

### 🎓 Student/Learner
1. Quick start: [QUICKSTART.md](QUICKSTART.md)
2. How to use: [DEMO_GUIDE.md](DEMO_GUIDE.md)
3. Learn more: [README.md](README.md)
4. Deep dive: [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)

## 🛠️ Quick Access Scripts

| File | Purpose |
|------|---------|
| `setup.bat` | Install all dependencies (Windows) |
| `start-servers.bat` | Start backend + frontend (Windows) |

## 📊 Documentation Stats

- **Total Docs**: 6 markdown files
- **Total Pages**: ~40 pages
- **Word Count**: ~8,000 words
- **Code Examples**: 20+
- **Visual Diagrams**: 5+

## 🔍 Search Guide

Looking for specific information?

| Topic | Found in |
|-------|----------|
| Installation | SETUP_GUIDE.md, README.md |
| Gestures | DEMO_GUIDE.md |
| Requirements | PROJECT_SUMMARY.md |
| File structure | PROJECT_STRUCTURE.md |
| Troubleshooting | SETUP_GUIDE.md, README.md |
| API endpoints | README.md, backend/server.js |
| Dependencies | README.md, PROJECT_STRUCTURE.md |
| Technology choices | PROJECT_SUMMARY.md |

## 💡 Recommended Reading Order

### First Time?
1. QUICKSTART.md (30 seconds)
2. DEMO_GUIDE.md (5 min)
3. README.md (10 min)

### Before Evaluation?
1. PROJECT_SUMMARY.md (8 min)
2. SETUP_GUIDE.md (3 min)
3. DEMO_GUIDE.md (5 min)

### Learning the Code?
1. PROJECT_STRUCTURE.md (7 min)
2. README.md (10 min)
3. Source code exploration

## 🎯 Key Features Summary

All docs reference these core features:

✅ **5 Pages**: Home, Research, Projects, About, Contact
✅ **5 Gestures**: Thumbs up, Peace, Open palm, Pointing, Fist
✅ **MediaPipe**: Camera-based hand tracking
✅ **Real-time**: Live gesture detection and feedback
✅ **Accessible**: Mouse, keyboard, and gesture control
✅ **Professional**: Modern UI with animations

## 🆘 Getting Help

1. **Setup issues**: SETUP_GUIDE.md → Troubleshooting
2. **Gesture problems**: DEMO_GUIDE.md → Tips
3. **Technical questions**: README.md → Technology Stack
4. **Requirements questions**: PROJECT_SUMMARY.md

## 📞 Project Links

- **Frontend**: http://localhost:5173 (after starting)
- **Backend**: http://localhost:5000 (after starting)
- **Health Check**: http://localhost:5000/api/health

---

## ✨ Ready to Start?

👉 **New user?** → [QUICKSTART.md](QUICKSTART.md)

👉 **Need to demo?** → [DEMO_GUIDE.md](DEMO_GUIDE.md)

👉 **Want full details?** → [README.md](README.md)

---

**Built with ❤️ for AI Center of Excellence**

*Last updated: January 26, 2026*
